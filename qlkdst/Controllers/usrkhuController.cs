...
namespace qlkdst.Controllers
{
    public class usrkhuController :BaseController
    {
        ...
        public ActionResult CreateMulti()
        {
            ...
            //if (!sUserName.ToLower().Equals("admin") && !sUserName.ToLower().Equals("trung") && sRole != "01" && sRole != "02")//nhom quan ly va admin thay het
            //{
            //    lstUsers = lstUsers.Where(x => x.username != "admin" && x.role == sRole && x.chinhanh == sChinhanh).ToList();
            //}

            ////an user admin di
            //if (!sUserName.ToLower().Equals("admin") && !sUserName.ToLower().Equals("trung"))
            //{
